...
/** A shelf row of books (hardcover spines) — the book sibling of MediaRow. */
export function BookRow({
  ...
  const listRef = useRef<FlatList<BookSummary>>(null);
  ...
  // Portrait covers; extra gap leaves room for the pages to fan out to the
  // right on hover without overlapping the next book.
  const gap = Math.round(posterWidth * 0.4);
  const coverHeight = posterWidth / aspect.book;
  const paddleTop = spacing.lg + coverHeight / 2 - 22;
      ...
      {loading ? (
        ...
        <View>
          <FlatList
            ...
            horizontal
            data={items}
            keyExtractor={(item) => item.volumeId}
            renderItem={({ item, index }) => (
              <SettleIn index={index}>
                <BookCard book={item} width={posterWidth} />
              </SettleIn>
            )}
            onScroll={onScroll}
